...
const Buildings = () => {
  ...
  const fetchProductCount = async () => {
    try {
      const response = await fetch(`${baseURL}buildings/get/count`);
      if (!response.ok) {
  ...
  const handleUpdateBoost = async (id) => {
    try {
      const response = await axios.put(`${baseURL}buildings/${id}/boost`);
  ...
  const handleUpdateApproval = async (id) => {
    try {
      const response = await axios.put(`${baseURL}buildings/${id}/approve`);
      const updatedProduct = response.data;
